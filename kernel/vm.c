...
void recurprint(pagetable_t page,char* prefix,int level){
  for(int i = 0;i < 512;i++){
    pte_t pte = page[i];
    if(pte & PTE_V){
      uint64 child = PTE2PA(pte);
      ...
      char child_prefix[12];
      memset(child_prefix,0,12);
      strcat(child_prefix,prefix,strlen(prefix));
      strcat(child_prefix + strlen(prefix)," ..",3);
      recurprint((pagetable_t)child,child_prefix,level + 1);
    }
  }
}
